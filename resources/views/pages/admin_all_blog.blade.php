<table class="table">
    ...
    @foreach($blog as $key => $val)
        <tr class="text-center">
            ...
            <td>{{$val->user_name}}</td>
            <td>
                <button id="" class="btn btn-danger btn_delBlog" data-blogid="{{$val->id}}" data-name="{{$val->name}}">Delete</button>
                <a href="/tin-tuc/edit/{{$val->id}}"><button id="" class="btn btn-warning btn_editblog" data-blogid="{{$val->id}}" data-name="{{$val->name}}">Edit</button></a>
            </td>
        </tr>
    @endforeach
